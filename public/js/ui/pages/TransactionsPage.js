...
class TransactionsPage {
  ...
  removeAccount() {
    ...
    if (confirm('Вы действительно хотите удалить счёт?')) {
      Account.remove({ id: this.lastOptions.account_id }, (err, resp) => {
        if (resp.success) App.updateWidgets();
      });
      this.clear();
    }
  }
  ...
  formatDate(date) {
    ...
    const day = newDate.toLocaleString('ru', {day: 'numeric', month: 'long', year: 'numeric'});
  ...
  renderTransactions(data) {
    if(data) {
      ...
      data.forEach((e) => {
        addListTransactions.insertAdjacentHTML('beforeend', this.getTransactionHTML(e));
      })
    }
  }
}